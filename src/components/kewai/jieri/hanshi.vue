	...
	<div>
		<section class="mainbox">
			<div class="column">
				<div class="ciyun">
					<div class="chart">
						<h2>动态词云图</h2>
						<div id="wrap">
						            <a href="#/primary1" class="tag">寒食</a>
						            <a href="#/primary1" class="tag">寒食日作</a>
						            <a href="#/primary1" class="tag">寒食书事</a>
						            <a href="#/primary1" class="tag">赵元镇</a>
						            <a href="#/primary1" class="tag">壬辰寒食</a>
						            <a href="#/primary1" class="tag">王安石</a>
						            <a href="#/primary1" class="tag">寒食上冢</a>
						            <a href="#/primary1" class="tag">诉衷情·寒食</a>
						            <a href="#/primary1" class="tag">周邦彦</a>
						            <a href="#/primary1" class="tag">琐窗寒·寒食</a>
						            <a href="#/primary1" class="tag">唐多令·寒食</a>
						            <a href="#/primary1" class="tag">朱孟德</a>
						            <a href="#/primary1" class="tag">李清照</a>
						</div>
					</div>
				</div>
			...
			<div class="column">
				<div class="btnlist">
					<div id="leibie">
						<el-button class="btn" round type="mini" @click="tochongyang">重阳</el-button>
						<el-button class="btn" round type="mini" @click="tochunjie">春节</el-button>
						<el-button class="btn" round type="mini" @click="toduanwu">端午</el-button>
						<el-button class="btn" round type="mini" @click="tohanshi">寒食</el-button>
						<el-button class="btn" round type="mini" @click="toqingming">清明</el-button>
						<el-button class="btn" round type="mini" @click="toqixi">七夕</el-button>
						<el-button class="btn" round type="mini" @click="toyuanxiao">元宵</el-button>
						<el-button class="btn" round type="mini" @click="tozhongqiu">中秋</el-button>
						<el-button class="btn" round type="mini" @click="tojieri">其他节日</el-button>
					</div>
					<div id="search">
				...
				<h2 style="display: inline-block;">寒食</h2>
				<div id="zoombtn">
...
export default{
	...
	methods:{
		...
		tochongyang:function(){
			this.$router.push('./chongyang')
		},
		tochunjie:function(){
			this.$router.push('./chunjie')
		},
		toduanwu:function(){
			this.$router.push('./duanwu')
		},
		tohanshi:function(){
			this.$router.push('./hanshi')
		},
		toqingming:function(){
			this.$router.push('./qingming')
		},
		toqixi:function(){
			this.$router.push('./qixi')
		},
		toyuanxiao:function(){
			this.$router.push('./yuanxiao')
		},
		tozhongqiu:function(){
			this.$router.push('./zhongqiu')
		},
		tojieri:function(){
			this.$router.push('./jieri')
		},
		initGraph(){
					var shuju;
					var lastFocusNode;
					...
					d3.json("static/json/kewai/jieri/寒食.json",function(error,data){
						shuju=data
						console.log(data)
						init(data)
						...
						/*$('#search input').keyup(function(event) {
							...
							if ($(this).val() == '') {
								d3.select('#svg1 .TEXT').selectAll('text').attr('class', '');
								d3.select('#svg1 .CIRCLE').selectAll('circle').attr('class', '');
								d3.select('#svg1 .LINE').selectAll('line').attr('class', '');
							}
							//否则判断判断三个元素是否等于name值，等于则显示该值
							else {
								var name = $(this).val();

							   d3.select('#svg1 .TEXT').selectAll('text').attr('class', function(d) {
											if (d.name.indexOf(name) >= 0||d.name.indexOf(name) >= 0) {
													return '';
											} else {
														//优化：与该搜索节点相关联的节点均显示
								...
								d3.select("#svg1 .LINK").selectAll('line').attr('class', function(d) {
									if (((data.nodes[d.source].name.indexOf(name) >= 0) ||
										((data.nodes[d.source].dynasty.indexOf(name) >= 0))
										)&&((data.nodes[d.target].name.indexOf(name) >= 0) ||
										((data.nodes[d.target].dynasty.indexOf(name) >= 0))
										)) {
										return '';
									} else {
										return 'inactive'; //隐藏
