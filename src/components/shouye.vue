	...
	methods:{
		...
		initGraph(){
			var shuju
					...
					d3.json("static/json/shouye.json",function(error,data){
						shuju=data
						console.log(data)
						init(data)
							...
							.style('fill',function(d,i){
								...
								for(let item of data.nodes){
									if(item.name == name||(item.dynasty == name&&!item.name)){
										...
										{
											$('#info1').append('<p><span>' + '关键字:' + '</span>' + '朝代' + '</p>');
										}
										else {
											if(!item.prop)$('#info1').append('<p><span>' + '关键字:' + '</span>' + '诗人</p>' +'<p><span>朝代:</span>'+ item.dynasty +'</p>');
											else $('#info1').append('<p><span>' + '关键字:' + '</span>' + '诗集' + '</p>');
										}
										for(var key in item.prop)
										//显示值及其字段名字
										$('#info1').append('<p><span>' + key + '</span>' + item.prop[key] + '</p>');
									}
								}
...
.mainbox{
	...
	/* min-width: 64rem;
	max-width: 120rem; */
	margin:0 auto;border-radius: 1.875rem;
