	...
	<div>
		<section class="mainbox">
			...
			<div class="column">
				...
				<h2 style="display: inline-block;">宋朝</h2>
				<div id="zoombtn">
...
export default{
	...
	methods:{
		...
		initGraph(){
					...
					d3.json("static/json/shiren/宋代诗人.json",function(error,data){
						shuju=data
						console.log(data)
						init(data)
						...
						//	绘制节点
							...
							.attr('r',function(d){
								if(!d.name){
									...
									var shi="市"
									var xian='县'
									if(d.dynasty.indexOf(sheng)>0)return 40;
									else return 30;
								}
								else return 25; //根据自身的索引号，自动随机获取一种背景颜色
										...
										if(!item.name)
											{
												...
												var shi="市"
												var xian='县'
												if(d.dynasty.indexOf(sheng)>0)$('#info1').append('<p><span>' + '关键字:' + '</span>' + '省级地区' + '</p>');
												else if(d.dynasty.indexOf(shi)>0 )$('#info1').append('<p><span>' + '关键字:' + '</span>' + '市级地区' + '</p>');
												else if(d.dynasty.indexOf(xian)>0 )$('#info1').append('<p><span>' + '关键字:' + '</span>' + '县级地区' + '</p>');
												else $('#info1').append('<p><span>' + '关键字:' + '</span>' + '不详' + '</p>');
												
											}
											else 
											{
												$('#info1').append('<p><span>' + '关键字:' + '</span>' + '诗人信息' + '</p>');
												$('#info1').append('<p><span>'+d.info+'</span></p>' );
											}	
									}
								}
							//显示随机图片
							...
							//当与连接点连接时变粗
								if ((line.source.name == name || line.target.name == name||line.source.dynasty == name || line.target.dynasty == name) ) {
			...
			mychart.setOption({
				...
				tooltip: {
				    ...
				    xAxis: [
				        {
				            type: 'category',
				            data: ['陆游', '苏轼', '刘克庄', '杨万里', '梅尧臣', '黄庭坚', '范成大','欧阳修','辛弃疾','王安石'],
				            axisTick: {
				                alignWithLabel: true
				            }
				        }
				    ...
				    series: [
				        {
				            ...
				            type: 'bar',
				            barWidth: '40%',
				            data: [10452, 3357 , 5453 , 5030 ,3222,2642,2323,1341,905,1931]
				        }
				    ]
			})
		}
